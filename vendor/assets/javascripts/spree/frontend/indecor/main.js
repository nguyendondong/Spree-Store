...
$('document').ready(function(){
    $('#checkout-now').on('click', function(e){
        e.preventDefault();
         $('form#checkout_form_address').submit();
         return false;
    })
 })
...
$('.title').on('keyup change', function(e){
    ...
    var per_product = ' per product'
    if (qty <= prices_array.length)
    {
        $('span.price.selling').text('$'+prices_array[qty -1] + per_product);
    }
    else
    {
        $('span.price.selling').text('$'+lowest_discounted_volume_price + per_product);
    }
});
