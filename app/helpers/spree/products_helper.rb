module Spree
  module ProductsHelper
    ...
    def cache_key_for_products
      ...
      current_store_id = current_store.present? ? current_store.id : 0
      max_updated_at = (@products.maximum(:updated_at) || Date.today).to_s(:number)
      products_cache_keys = "#{I18n.locale}/#{current_currency}/spree/products/all-#{current_store_id}-#{params[:page]}-#{max_updated_at}-#{count}"
      products_cache_keys
    end
  end
end