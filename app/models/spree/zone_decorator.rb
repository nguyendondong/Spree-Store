  ...
  Zone.class_eval do
    ...
    def self.match(address)
      return unless address and matches = self.includes(:zone_members).
        ...
        where("(spree_zone_members.zoneable_type = 'Spree::Country' AND spree_zone_members.zoneable_id = ?) 
          OR (spree_zone_members.zoneable_type = 'Spree::State' AND spree_zone_members.zoneable_id = ?)
          OR (spree_zone_members.zoneable_type = 'Spree::District' AND spree_zone_members.zoneable_id = ?)", address.country_id, address.state_id, address.district_id).
        references(:zones)

      ['district', 'state', 'country'].each do |zone_kind|
    ...
    def include?(address)
      ...
      members.any? do |zone_member|
        ...
        when 'Spree::District'
          zone_member.zoneable_id == address.district_id
        else
          false
        end
      end
    ...
    def district_ids=(ids)
      ...
      ids.reject{ |id| id.blank? }.map do |id|
        member = ZoneMember.new
        member.zoneable_type = 'Spree::District'
        member.zoneable_id = id
        members << member
      end
    end
    ...
    # the check.
    def contains?(target)
      return false if (kind == 'state' && target.kind == 'country') || (kind == 'district' && target.kind == 'state')
