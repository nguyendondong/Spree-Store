$ ->
  ...
  class TransferLocations
    ...
    toggle_source_location: (hide=false) ->
      ...
      if @is_source_location_hidden() and not hide
        ...
        $('#stock_movement_quantity_field').show()
        $('#item_quantity').val('0')
      else
        $('#transfer_source_location_id_field').css('visibility', 'hidden')
        $('#stock_movement_quantity_field').hide()
    ...
    populate_select: (select, except=0) ->
      ...
      if select.attr("id") == 'transfer_source_location_id' && select.attr("selected_id") != undefined && except == 0
        for location in @locations when location.id isnt except
          if location.id == parseInt(select.attr("selected_id"))
            select.append $('<option></option>').text(location.name).attr('value', location.id)
            break
        for location in @locations when location.id isnt except && location.id != parseInt(select.attr("selected_id"))
          select.append $('<option></option>').text(location.name).attr('value', location.id)
      else
        for location in @locations when location.id isnt except
          select.append $('<option></option>').text(location.name).attr('value', location.id)
  ...
  class TransferAddVariants
    constructor: ->
      ...
      $('button.transfer_add_variant').click (event) =>
        ...
        if $('#transfer_variant').select2('data')?
          if (parseInt($('#item_quantity').val()) < parseInt($('#transfer_variant_quantity').val()) || parseInt($('#transfer_variant_quantity').val()) <= 0) && !$( "#transfer_receive_stock" )[0].checked
    ...
    add_variant: ->
      ...
      rest = parseInt($('#item_quantity').val()) - quantity
      $('#item_quantity').val(rest)
      @render()
